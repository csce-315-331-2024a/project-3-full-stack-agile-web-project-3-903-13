...
export default function EmployeeNavbar({ links }) {
  ...
  return (
    <nav className="flex w-full h-[5rem] bg-white shadow-md">
      ...
      <div className="absolute right-[1.5rem] md:hidden group" onClick={menuToggle}>
          ...
          <ul className={`${isOpen ? "block bg-white border shadow mr-1" : "hidden"} absolute rounded-xl md:shadow-none md:bg-none md:border-0 md:relative right-0 md:mt-0 p-4 md:p-0 md:flex space-y-6 md:space-y-0 md:space-x-8 text-sm md:text-base`} style={{ marginTop: `${links.length * 3 + 1}rem` }}>
            {links.map((link) => (
              <li key={link.route}>
                <Link
                  className={
