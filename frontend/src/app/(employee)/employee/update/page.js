...
function UpdateOrder({ components, setComponents, shallowCopy, setShallowCopy }) {
    ...
    */
    ...
    const updatePlacedOrder = async () => {
        const response = await fetch("https://project-3-full-stack-agile-web-project-3-lc1v.onrender.com/api/transactions/updateTransaction", {
            method: "PATCH",
            headers: {
                "Content-Type": "application/json",
            },
            body: JSON.stringify({ transactionID: id , oldcomponents: shallowCopy, newComponents: components}),
        }
        );
        setComponents();
        setShallowCopy();
    ...
    const getCharge = () => {
        return (components.reduce((total, item) => total + item.price * item.quantity, 0)
        ...
        <div className="flex flex-col grow border-2 border-gray-400 rounded-lg shadow-lg mr-5">
            ...
            <div className="px-6 py-4">
                {components === null ? (
                    ...
                    getCharge() >= 0 ? (
                        <div className="font-semibold text-lg" >Price to pay: ${getCharge().toFixed(2)}</div>
                    ) : (
                        <div className="font-semibold text-lg" >Refund: ${(-1 * getCharge()).toFixed(2)}</div>
                    )
                )}
            </div>
            ...
            <div className="px-6 py-4 flex flex-col space-y-2">
                ...
                {
                    getCharge() < 0 && (
                        <button className="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded shadow" onClick={handleRefund}>
                            Update and refund
                        </button>
                    )
                }
                ...
                {message.length > 0 && (
                    <div className="text-lg font-semibold text-green-500"> {message} </div>
                )}
...
function MenuItem(props) {
    ...
    const sendToTransaction = () => {
        ...
        updateTransaction({ "id": item.menuid, "itemname": item.itemname, "price": item.price, "quantity": quantity });
        setIsClicked(true);
...
export default function EmployeePOSPage() {
    ...
    if (errorMessage && errorMessage.length > 0) {
        return <div className="text-2xl text-red-500 font-bold my-6 text-center"> {errorMessage} </div>
    }
    ...
    return (
        <main className="min-h-screen bg-cream py-1">
            <div className="flex flex-row h-[90vh]">
                ...
                <UpdateOrder components={components} setComponents={setComponents} shallowCopy={shallowCopy} setShallowCopy={setShallowCopy} />
            </div>
